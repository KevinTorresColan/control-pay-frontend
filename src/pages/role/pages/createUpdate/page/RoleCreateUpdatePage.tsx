...
import { homeRoutes, roleRoutes } from "@/routes";
...
import { IRoleCreateUpdate } from "@/pages/role/interfaces";
import { schemaCreateUpdate } from "@/pages/role/schemas";

const RoleCreateUpdatePage = () => {
  const { createRole, isLoadingCreate, isLoadingUpdate, updateRole, roleById } =
    useRole();
  ...
  const ref = useRef<IFormWrapperRef<IRoleCreateUpdate>>(null);
  const form = useForm();
  const { setValue } = form;
  ...
  const links: Ilink[] = [
    ...
    { name: "Listar roles", href: roleRoutes.get },
    { name: id ? "Actualizar rol" : "Crear rol" },
  ];

  const handleAlert = (data: IApiResponse<IRoleCreateUpdate>) => {
    generateAlert({ ...data });
    return data;
  };

  const onSubmitSuccess = async (dataSend: IRoleCreateUpdate) => {
    if (id) await updateRole(id, dataSend).then(pipe(handleAlert));
    else await createRole(dataSend).then(pipe(handleAlert));
    setOpen(false);
  };
    ...
    <>
      ...
      >
        ...
        <Button
          variant="outlined"
          sx={{ order: { xs: 2, sm: 1 } }}
          onClick={() => navigate(roleRoutes.get)}
